...
/***/ function(module, exports, __webpack_require__) {
	...
	var instrument = audioContext.createOscillator();
	...
	var getBandwidth = function getBandwidth(analyser, freqs) {
	...
	var freqToIndex = function freqToIndex(analyser, freq) {
	  var nyquist = audioContext.sampleRate / 2;
	  return Math.round(freq / nyquist * analyser.fftSize / 2);
	};
	
	var indexToFreq = function indexToFreq(analyser, index) {
	  var nyquist = audioContext.sampleRate / 2;
	  return nyquist / (analyser.fftSize / 2) * index;
	};
	
	var readMic = function readMic(analyser) {
	  ...
	  if (window.scrollDemoIsActive && (band[0] > 4 || band[1] > 4)) {
	    ...
	    var scale = 10;
	    (0, _jquery2.default)(window).scrollTop(currentScroll + scale * bandwidthDifference);
	  }
	};
	
	var optimizeFrequency = function optimizeFrequency(osc, analyser, freqSweepStart, freqSweepEnd) {
	...
	var handleMic = function handleMic(stream, callback) {
	  // Mic
